...
function mapServerPostToBlogPost(serverPost: ServerPost, locale: string): BlogPost {
  return {
    locale: locale,
    ...
    status: (serverPost.status as 'published' | 'draft' | 'archived') ?? "published",
    is_pinned: serverPost.is_pinned ?? false,
    content: serverPost.content ?? '',
    visibility: (serverPost.visibility as 'public' | 'logged_in' | 'subscribers') ?? 'public',
  };
}
...
export async function getPostBySlug(
        ...
        if (localSlug === targetSlug && data.status !== 'draft') {
          return {
            post: {
              ...
              published_at: data.published_at ? new Date(data.published_at) : new Date(),
              status: (data.status as 'published' | 'draft' | 'archived') || 'published',
              visibility: (data.visibility as 'public' | 'logged_in' | 'subscribers') || 'public',
              is_pinned: data.is_pinned || false,
              content,
              metadata: data,
            },
  ...
  if (serverResult.success && serverResult.data) {
    return {
      post: mapServerPostToBlogPost(serverResult.data as any, locale),
      error: undefined,
      errorCode: serverResult.customCode,
    };
  } else if (!serverResult.success) {
    return { post: null, error: (serverResult as any).error, errorCode: serverResult.customCode };
  } else {
