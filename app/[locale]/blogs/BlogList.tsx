...
function mapServerPostToBlogCard(post: PublicPost, locale: string): BlogPost {
  return {
    ...
    visibility: (post.visibility as "public" | "logged_in" | "subscribers") ?? "public",
...
export function BlogList({
  ...
  const loadMorePosts = useCallback(async () => {
    ...
    if (result.success && (result.data as any)?.posts) {
      const newPosts = (result.data as any).posts;
      const newTotal = (result.data as any).count ?? initialTotal;
      setPosts((prevPosts) => [...prevPosts, ...newPosts]);
      setPageIndex((prevIndex) => prevIndex + 1);
      setHasMore(posts.length + newPosts.length < newTotal);
    } else {
      console.error("Failed to load more posts:", (result as any).error);
  ...
  const handleTagSelect = async (tagId: string | null) => {
    ...
    if (result.success && (result.data as any)?.posts) {
      ...
      setPageIndex(1);
      setHasMore((result.data as any).posts.length < ((result.data as any).count ?? 0));
    } else {
      console.error("Failed to filter posts by tag:", (result as any).error);
